...
//!    // Current options for shape is only "circles" and "squares".
...
impl Default for PlotSettings {
    fn default() -> PlotSettings {
        ...
        PlotSettings {
            plot_color: Color::set_vals(50, 50, 230),
            shape_type: custom_string,
            line_thickness: 2,
        }
    }
}
...
impl Custom for PlotSettings {
    fn custom_plot(r: u8, g: u8, b: u8, _shape_type: String, _line_thickness: u8) -> PlotSettings {
        PlotSettings {
            plot_color: Color::set_vals(r, g, b),
...
impl GetOffsetValues for OffsetValues {
    fn get_offset(pair_x: &Bounds, pair_y: &Bounds) -> OffsetValues {
        ...
        OffsetValues {
            origin_x: (origin_x_temp),
            origin_y: (origin_y_temp),
        }
    }
}
...
fn shrink_vec(vec: &Vec<i32>) -> Vec<i32> {
    ...
    for i in vec {
        ...
        if i.is_negative() {
            let temp_num = round((switch_sign(&(*i)) as f32).log(1.05), 0) as i32; // Change hard coded 1.05 to something more dynamic in the future
...
pub fn plot_tbl(_vec_x: &Vec<i32>, _vec_y: &Vec<i32>, settings: &PlotSettings, source: &str) {
    ...
    if vec_x.len() != vec_y.len() {
        panic!("Error: Vector X and Y does not have the same number of elements!");
    }

    let bounds_x = Bounds {
        smallest: find_smallest_elem(&vec_x),
        largest: find_largest_elem(&vec_x),
    };
    let bounds_y = Bounds {
        smallest: find_smallest_elem(&vec_y),
        largest: find_largest_elem(&vec_y),
    };
    ...
    for i in 0..vec_x.len() {
        for j in get_elem_pos(
            ...
            *pixel = image::Rgb([
                settings.plot_color.red,
                settings.plot_color.green,
                settings.plot_color.blue,
            ]);
        }
    }
...
/// Helper function generating a map of instructions representing the individual elements
fn gen_cir_map(x_pos: &i32, y_pos: &i32) -> Vec<Vector2D<i32>> {
    let mut map = Vec::new();

    let c_p: Vector2D<i32> = Vector2D {
        x: x_pos + (CIRCLE_RADIUS / 2),
        y: y_pos + (CIRCLE_RADIUS / 2),
    };
    ...
    for i in *x_pos..c_p.x + CIRCLE_RADIUS {
        for j in *y_pos..c_p.y + CIRCLE_RADIUS {
            ...
            println!(
                "diameter: {}",
                ((x_pos - i).pow(2) as f64 + (y_pos - j).pow(2) as f64).sqrt()
            );
            if ((c_p.x - i).pow(2) as f64 + (c_p.y - j).pow(2) as f64).sqrt()
                < (CIRCLE_RADIUS as f64).sqrt()
            {
                let element: Vector2D<i32> = Vector2D {
                    x: i - 12,
                    y: j - 11,
                };
                map.push(element);
